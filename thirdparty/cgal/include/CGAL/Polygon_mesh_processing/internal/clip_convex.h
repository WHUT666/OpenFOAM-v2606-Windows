...
//
// $URL: https://github.com/CGAL/cgal/blob/v6.2.1/PMP_Boolean_operations/include/CGAL/Polygon_mesh_processing/internal/clip_convex.h $
// $Id: include/CGAL/Polygon_mesh_processing/internal/clip_convex.h 28811b671a1 $
...
template<class Visitor, class PolygonMesh>
struct Wrap_visitor_for_triangulation: public Triangulate_faces::Default_visitor<PolygonMesh>
{
  Wrap_visitor_for_triangulation(const PolygonMesh &pm, Visitor &v): pm(pm), visitor(v){}

  typedef typename boost::graph_traits<PolygonMesh>::face_descriptor face_descriptor;
  typedef typename boost::graph_traits<PolygonMesh>::vertex_descriptor vertex_descriptor;

  void before_subface_creations(face_descriptor f_old) {
    visitor.before_subface_creations(f_old, pm);
  }
  void after_subface_creations() {
    visitor.after_subface_creations(pm);
  }
  void after_subface_created(face_descriptor f_new) {
    visitor.after_subface_created(f_new, pm);
  }
...
/**
  ...
  * Go in the opposite direction, use only for input with boundaries
  */
...
refine_convex_with_plane_using_opposite_direction(PolygonMesh& pm,
                                                  const Plane_3& plane,
                                                  ...
                                                  const NamedParameters& np = parameters::default_values())
{
  ...
  do{
    ...
    CGAL_assertion(oriented_side(plane, get(vpm, source(h,pm)))==ON_NEGATIVE_SIDE);
    ...
    if(side_trg != ON_POSITIVE_SIDE){
      ...
      // if h is a border, end the function
      if(is_border(h, pm))
        return boundaries;
      continue;
    }
    ...
    h = next(h, pm);
    side_trg=oriented_side(plane, get(vpm, target(h,pm)));
    while(side_trg == ON_POSITIVE_SIDE){
      h = next(h,pm);
      side_trg=oriented_side(plane, get(vpm, target(h,pm)));
    }
    ...
    if (triangulate){
      ...
      if(!is_triangle(sh_opp, pm)){
        ...
        if constexpr(is_f2f_map)
          put(f2f, face(opposite(newh, pm), pm), get(f2f, face(newh, pm)));
                         ...
                         const NamedParameters& np = parameters::default_values())
{
  ...
  do{
    ...
    if(side_trg != ON_NEGATIVE_SIDE){
      ...
      // Specific case with boundaries, we explore in the opposite direction
      if(is_border(h, pm)){
        auto other_direction_boundaries = refine_convex_with_plane_using_opposite_direction(pm, plane, opposite(h_start, pm), np);
        std::reverse(other_direction_boundaries.begin(), other_direction_boundaries.end());
        other_direction_boundaries.insert(other_direction_boundaries.begin(), boundaries.begin(), boundaries.end());
        return boundaries;
      }
      continue;
    }
                               ...
                               typename boost::graph_traits<PolygonMesh>::face_descriptor plane_fd = boost::graph_traits<PolygonMesh>::null_face())
{
  ...
  if (clip_volume && faces(pm).size()>1){ // If there is only one face, the output is flat does not need to be closed
    visitor.before_face_copy(BGT::null_face(), pm, pm);
    ...
    visitor.after_face_copy(BGT::null_face(), pm, f, pm);

    if(triangulate){
      if constexpr(has_visitor){
        Wrap_visitor_for_triangulation wrap_visitor(pm, visitor);
        triangulate_face(f, pm, parameters::vertex_point_map(vpm).visitor(wrap_visitor));
      } else {
...
clip_convex(PolygonMesh& pm,
  ...
  if(vertices(pm).size()==1){
    ...
    // Dimension == 2 with only one face
    ...
    if(src_split != BGT::null_halfedge() && split_edge != BGT::null_halfedge()){
      ...
      if(triangulate){
        if constexpr(has_visitor){
          Wrap_visitor_for_triangulation wrap_visitor(pm, visitor);
          triangulate_face(face(split_edge, pm), pm, parameters::vertex_point_map(vpm).visitor(wrap_visitor));
        } else {
          triangulate_face(face(split_edge, pm), pm, parameters::vertex_point_map(vpm));
        }
      }
    }
