...
//
// $URL: https://github.com/CGAL/cgal/blob/v6.2.1/Surface_sweep_2/include/CGAL/Surface_sweep_2/Default_subcurve.h $
// $Id: include/CGAL/Surface_sweep_2/Default_subcurve.h 28811b671a1 $
 ...
 */
template <typename GeometryTraits_2, typename Event_, typename Allocator_, typename Subcurve_>
class Default_subcurve_base : public No_overlap_subcurve<GeometryTraits_2, Event_, Allocator_, Subcurve_> {
public:
  using Geometry_traits_2 = GeometryTraits_2;
  using Subcurve = Subcurve_;
  using Event = Event_;
  using Handle_overlaps = Tag_true;

private:
  using Gt2 = Geometry_traits_2;
  using Base = No_overlap_subcurve<Gt2, Event, Subcurve>;
  using Self = Default_subcurve_base<GeometryTraits_2, Event_, Allocator_, Subcurve_>;
  using Intersected_set = Small_unordered_set<Self*, 8>;
...
public:
  ...
  /*! obtains the subcurves that originate an overlap.
   */
  ...
  void set_originating_subcurve1(Subcurve* orig_subcurve1) { m_orig_subcurve1 = orig_subcurve1; }

  void set_originating_subcurve2(Subcurve* orig_subcurve2) { m_orig_subcurve2 = orig_subcurve2; }

  /*! obtains all the leaf-nodes in the hierarchy of overlapping subcurves. */
  ...
  /*! checks whether the given subcurve is a node in the overlapping hierarchy.
   */
  bool is_inner_node(Subcurve* s) {
    if (this == s) return true;
    if (m_orig_subcurve1 == nullptr) return false;
    return (m_orig_subcurve1->is_inner_node(s) || m_orig_subcurve2->is_inner_node(s));
  }
  ...
  /*! checks whether other is entirely contained in the hierarchy of subcurves
   * of this.
   */
  bool are_all_leaves_contained(Subcurve* other) {
    ...
    for (auto it = leaves_of_other.begin(); it != leaves_of_other.end(); ++it) {
  ...
  /*! checks whether the two hierarchies contain the same leaf nodes.
   */
  bool has_same_leaves(Subcurve* s) {
    ...
    for (auto iter = my_leaves.begin(); iter != my_leaves.end(); ++iter) {
      if (std::find(other_leaves.begin(), other_leaves.end(), *iter) == other_leaves.end()) return false;
    }

    for (auto iter = other_leaves.begin(); iter != other_leaves.end(); ++iter) {
      if (std::find(my_leaves.begin(), my_leaves.end(), *iter) == my_leaves.end()) return false;
    }

    return true;
  }

  /*! checks whether the two hierarchies (s1+s2 considered as an overlapping
  ...
  /*! checks whether the two hierarchies contain a common leaf node. */
  bool has_common_leaf(Subcurve* s) {
    ...
    for (auto iter = my_leaves.begin(); iter != my_leaves.end(); ++iter) {
      if (std::find(other_leaves.begin(), other_leaves.end(), *iter) != other_leaves.end()) return true;
    }
    return false;
  }

  /*! obtains all distinct nodes from the two hierarchies. */
  template <typename OutputIterator>
  ...
  /*! obtains the depth of the overlap hierarchy.
   */
  std::size_t overlap_depth() {
    if (m_orig_subcurve1 == nullptr) return (1);

    auto depth1 = m_orig_subcurve1->overlap_depth();
    auto depth2 = m_orig_subcurve2->overlap_depth();
    if (depth1 > depth2) return (depth1 + 1);
    else return (depth2 + 1);
  }

  /*! obtains the number of input curves contributing to the subcurve
   */
  std::size_t number_of_original_curves() const {
    if (m_orig_subcurve1 == nullptr) return 1;
    auto d1 = m_orig_subcurve1->number_of_original_curves();
    auto d2 = m_orig_subcurve2->number_of_original_curves();
    return d1+d2;
  }
};
...
class Default_subcurve :
  public Default_subcurve_base<
    GeometryTraits_2, Event_, Allocator_,
    typename Default::Get<Subcurve_,  Default_subcurve<GeometryTraits_2, Event_, Allocator_, Subcurve_>>::type> {
public:
  using Geometry_traits_2 = GeometryTraits_2;
  using Event = Event_;
...
private:
  using Gt2 = Geometry_traits_2;
  using Self = Default_subcurve<Gt2, Event, Allocator, Subcurve_>;
  using Subcurve = typename Default::Get<Subcurve_, Self>::type;
  using Base = Default_subcurve_base<Gt2, Event, Allocator, Subcurve>;
